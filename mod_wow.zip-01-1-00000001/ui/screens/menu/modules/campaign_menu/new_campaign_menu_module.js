...
NewCampaignMenuModule.prototype.createDIV = function (_parentDiv)
{
    ...
    contentContainer.append(this.mThirdPanel);
		...
		var ultraDifficultyControl = $('<div class="control"></div>');
		row.append(ultraDifficultyControl);
		this.mDifficultyUltraCheckbox = $('<input type="radio" id="cb-difficulty-ultra" name="difficulty" />');
		ultraDifficultyControl.append(this.mDifficultyUltraCheckbox);
		this.mDifficultyUltraLabel = $('<label class="text-font-normal font-color-subtitle" for="cb-difficulty-ultra">Ultra</label>');
		ultraDifficultyControl.append(this.mDifficultyUltraLabel);
		this.mDifficultyUltraCheckbox.iCheck({
			checkboxClass: 'icheckbox_flat-orange',
			radioClass: 'iradio_flat-orange',
			increaseArea: '30%'
		});
		this.mDifficultyUltraCheckbox.on('ifChecked', null, this, function (_event)
		{
			var self = _event.data;
			self.mDifficulty = 3;
		});
...
NewCampaignMenuModule.prototype.destroyDIV = function ()
{
	...
	this.mDifficultyHardLabel = null;
	this.mDifficultyUltraCheckbox.remove();
	this.mDifficultyUltraCheckbox = null;
