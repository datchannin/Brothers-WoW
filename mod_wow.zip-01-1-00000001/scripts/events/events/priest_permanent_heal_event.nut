...
this.priest_permanent_heal_event <- this.inherit("scripts/events/event", {
	m = {
		Priest = null,
		Other = null
	},
	function create()
	{
		this.m.ID = "event.priest_permanent_heal";
		this.m.Title = "During camp...";
		this.m.Cooldown = 90.0 * this.World.getTime().SecondsPerDay;
		this.m.Screens.push({
			ID = "A",
			Text = "[img]gfx/ui/events/world/event_priest_permanent_heal.png[/img]{%priest% has enough strength to heal one of the permanent injuries in the squad!}",
			Image = "",
			List = [],
			Characters = [],
			Options = [],
			function start( _event )
			{
				...
				_event.m.Priest.improveMood(3.0, "Healed the permanent injury");
				...
				_event.m.Priest.addXP(1000, false);
				this.List.push({
					id = 5,
					icon = "ui/icons/xp_received.png",
					text = _event.m.Priest.getName() + " gains [color=" + this.Const.UI.Color.PositiveEventValue + "]+1000[/color] Experience"
				});
				_event.m.Priest.updateLevel();

				local injuries = _event.m.Other.getSkills().query(this.Const.SkillType.PermanentInjury);
