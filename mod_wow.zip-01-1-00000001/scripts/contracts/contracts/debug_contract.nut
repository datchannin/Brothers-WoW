...
this.debug_contract <- this.inherit("scripts/contracts/contract", {
	...
	function start()
	{
		...
		this.m.Flags.set("BossName", "DEBUGGER");
		this.m.Flags.set("LocationName", "DEBUGGER\'s Refuge");
	...
	function createStates()
	{
		this.m.States.push({
			...
			function onAIDeployment()
			{
				...
				e.getSkills().update();
				e.setHitpoints(5);
				e = this.Tactical.spawnEntity("scripts/entity/tactical/enemies/necromancer", 18, 17 - 18 / 2);
				...
				e.getSkills().update();
				e = this.Tactical.spawnEntity("scripts/entity/tactical/enemies/necromancer", 17, 18 - 17 / 2);
				e.setFaction(this.Const.Faction.Enemy);
				e.getAIAgent().getProperties().OverallDefensivenessMult = 0.0;
				e.getAIAgent().getProperties().BehaviorMult[this.Const.AI.Behavior.ID.Retreat] = 0.0;
				e.assignRandomEquipment();
