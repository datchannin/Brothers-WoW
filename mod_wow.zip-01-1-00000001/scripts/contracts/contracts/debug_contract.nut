	...
	function createStates()
	{
		this.m.States.push({
			...
			function onAIDeployment()
			{
				...
				e.getSkills().update();
				e.setHitpoints(5);
				e = this.Tactical.spawnEntity("scripts/entity/tactical/enemies/hexe", 18, 17 - 18 / 2);
				...
				e.getSkills().update();
				e = this.Tactical.spawnEntity("scripts/entity/tactical/enemies/hexe", 17, 18 - 17 / 2);
				e.setFaction(this.Const.Faction.Enemy);
				e.getAIAgent().getProperties().OverallDefensivenessMult = 0.0;
				e.getAIAgent().getProperties().BehaviorMult[this.Const.AI.Behavior.ID.Retreat] = 0.0;
				e.assignRandomEquipment();
