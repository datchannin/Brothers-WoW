...
this.paladin_concentrationaura_skill <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Description = "Apply Concentration Aura on the Paladin. This aura increases Fatigue Recovery of all party members near Paladin.";
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = "[color=" + this.Const.UI.Color.PositiveValue + "]+" + total_value + "[/color] Fatigue Recovery per turn for all party members within [color=" + this.Const.UI.Color.PositiveValue + "]" + total_radius + "[/color] tiles"
		});
